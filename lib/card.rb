class Card
  ...
  def self.is_set?(card1, card2, card3)
    NUMBERS.all? { |attr| all_same_or_all_different?(card1.number, card2.number, card3.number) } &&
    SHAPES.all? { |attr| all_same_or_all_different?(card1.shape, card2.shape, card3.shape) } &&
    SHADINGS.all? { |attr| all_same_or_all_different?(card1.shading, card2.shading, card3.shading) } &&
    COLORS.all? { |attr| all_same_or_all_different?(card1.color, card2.color, card3.color) }
  end
